...
$(document).ready(function(){
    ...
    $("#checkAll").click(function(){
        if($("#checkAll").is(':checked') ) {
            $("#question_type_privateFor > option").prop("selected", "selected");
            $("#question_type_privateFor").trigger("change");
        } else {
            $("#question_type_privateFor > option").removeAttr("selected");
            $("#question_type_privateFor").trigger("change");
        }
    });
	...
	$('.btn-comment-reply').click(function () {
        var idToOpen;
        if($(this).hasClass('btn-question-comment-reply')) {
            idToOpen = '#qc'+$(this).data('comment-open');
        } else {
            idToOpen = '#ac'+$(this).data('comment-open');
        }
		var replyToUserId = $(this).data('reply');

		if($(idToOpen).is(":visible")) {
			$(idToOpen).slideUp();
			return false;
		} else {
			$(idToOpen).slideDown();
		}
		$(idToOpen).find('.qa-form-tall-text').focus();
		$(idToOpen).find('.comment-for-user-input').val(replyToUserId);
	});
	$('#scrollToAnswer').click(function () {
		$('html,body').animate({
			scrollTop: $("#anew").offset().top
		}, 800);
		$('#a_content').focus();
